...
with tab_ask:
    ...
    if "ask_context" not in st.session_state:
        ...
        st.session_state.ask_context = context_str
    ...
    if prompt := st.chat_input("e.g., What time is soccer on Saturday?"):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                response = ask_ai(prompt, st.session_state.ask_context)
            st.write(response)
...
if st.sidebar.button("Refresh Data"):
    ...
    if "ask_context" in st.session_state:
        del st.session_state.ask_context
    st.rerun()
